...
```bash
# Jupyter notebook (run load_tmdb.ipynb to download TMDb and explore)
devbox run notebook
```
...
- **`load_tmdb.ipynb`** — Main entry point: downloads the TMDb dataset via `kagglehub.dataset_download()`, resolves zip or extracted directory, reads the first CSV, then keyword extraction, NRC Emotion Lexicon scoring (word-level sentiment/emotion pivot), and NRC Emotion Intensity Lexicon scoring.
